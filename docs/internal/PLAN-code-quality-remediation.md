...
- [~] **MED — DOM XSS** in mapper panes — **PARTIAL (step 2a):** `escHtml` unified to the null-safe, quote-escaping escaper (hardens text + attribute-VALUE contexts). **STILL OPEN:** inline `onclick="fn('${name}')"` handlers — HTML-escaping cannot fix this vector (parser decodes entities before JS runs); needs `addEventListener` + `dataset` refactor across source/target/edge/menu renderers. **Effort M, Risk Med.**
