...
| :--- | :--- | :--- |
| P12.1 | **Runtime mapping export (the durable contract).** Expose the existing mapping (concept→table, property→column, field expressions) as a versioned, load-independent artifact — **OSI/YAML and R2RML** (R2RML is what VKG engines like Ontop consume; ADR-0001) — consumable by the fabric's mapping layer. Include a content hash so consumers can cite the mapping version used for a query. | Mostly packaging of `generate-config` output + an R2RML serializer; no loader dependency. This is what the fabric depends on regardless of the P12.2 buy-vs-build outcome. |
| P12.2 | **Pushdown query generation.** Given a mapping + a relational query-graph **partition** (resolved concept/properties + predicate) from the fabric's conceptual-query IR (ADR-0001), generate a **parameterized SQL query** in the source dialect (PostgreSQL first) that returns only the requested rows/columns. Filters must push down; no client-side scans. | **Build-vs-buy (ADR-0001):** a VKG engine (**Ontop**) already does SPARQL→SQL pushdown with no materialization across all supported sources, driven by the P12.1 R2RML export — so treat native generation as a **stopgap/fallback** unless the team decides against running a VKG engine. If built: compile field expressions per dialect; reuse the Phase 5c expression engine. |
