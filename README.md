...
| `ontology suggest` | **(AI, Phase 10)** Ask an LLM to propose a richer target ontology (vertex-vs-edge, implicit relationships, clearer names) from the snapshot; prints the proposal + a diff vs the current mapping + validation/provenance notes. Metadata-only (Phase-9 Restricted/PII columns redacted, never sampled); every proposal is validated/repaired against the real schema so it always yields a loadable mapping. Nothing is written without `--apply`. Providers: OpenAI, Anthropic, and OpenAI-compatible/local (Ollama, vLLM, LM Studio, …) via `--provider` + `--base-url`; `--sample` grounds the model with a few example values from **non-sensitive** columns only, and `--ground` adds deterministic Phase-11 denormalization findings as advisory evidence (`--domain`, `--model`, `--api-key`, `--samples-per-column`, `--apply`, `--yes`, `--json`). Needs `r2g-arango[llm]` + the provider's key (`$OPENAI_API_KEY` / `$ANTHROPIC_API_KEY`; a local endpoint may need none) |
...
- **LLM-assisted ontology derivation (Phase 10):** an *optional* way to ask a model (`r2g-arango[llm]`) to **propose** a richer target graph from the introspected schema — the LLM proposes, the deterministic pipeline disposes. Works with OpenAI, Anthropic (Claude), or any OpenAI-compatible/local endpoint (Ollama, vLLM, LM Studio, …). Metadata-only (no row data; Phase-9 Restricted/PII columns are redacted to name-only and never sampled), prompt-injection-hardened, and validated/repaired against the real schema so a proposal can never load a hallucinated table or column. Opt-in `--sample` grounds the model with a few example values from **non-sensitive** columns only, and `--ground` feeds it deterministic Phase-11 denormalization findings as advisory evidence. The proposal flows through the **same** `validate_config` → mapper-review → loader path as Auto-Map, which stays the default; nothing is applied without explicit confirmation. Use it from the CLI (`r2g ontology suggest`) or the **Mapping Studio**: a "Suggest model (AI)" action (Actions menu / canvas right-click / `m`) opens a floating review panel where you accept or reject each suggestion per item, then apply the selection as an editable draft to review and Save.
