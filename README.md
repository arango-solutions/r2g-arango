...
- **LLM-assisted ontology derivation (Phase 10a):** an *optional* `r2g ontology suggest` that asks a model (OpenAI; `r2g-arango[llm]`) to **propose** a richer target graph from the introspected schema — the LLM proposes, the deterministic pipeline disposes. Metadata-only (no row data; Phase-9 Restricted/PII columns are redacted to name-only and never sampled), prompt-injection-hardened, and validated/repaired against the real schema so a proposal can never load a hallucinated table or column. The proposal flows through the **same** `validate_config` → mapper-review → loader path as Auto-Map, which stays the default; nothing is applied without explicit confirmation. Studio review/apply (10b) and opt-in sampling / more providers (10c) are planned.
